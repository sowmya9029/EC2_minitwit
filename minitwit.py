...
def user_timeline(username):
    ...
    if g.user:
        followed = query_db('''select 1 from follower where
            ...
            (session['user_id'], profile_user[0]['user_id'])) is not None
    return render_template('timeline.html', messages=query_db('''
            select message.*, user.* from message, user where
            user.user_id = message.author_id and user.user_id = %s
            order by message.pub_date desc limit %s''' %
            (profile_user[0]['user_id'], PER_PAGE)), followed=followed,
...
def follow_user(username):
    ...
    if not g.user:
        abort(401)
    whom_id = get_user_id(username)['user_id']
    if whom_id is None:
        abort(404)
    db = get_db()

    db.cursor().execute("insert into follower (who_id, whom_id) values (%d, %d)" %
